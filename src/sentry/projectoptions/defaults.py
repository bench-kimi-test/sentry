...
# mapping embedded in the PDB, and to return the per-frame VCS `revision`.
# Opt-in because rewriting `abs_path` can change downstream behaviors such as
# code mapping resolution and source link generation.
register(key="sentry:apply_pdb_srcsrv", default=False)
